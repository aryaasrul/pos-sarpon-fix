...
function formatPriceDisplay(menuItem) {
  ...
  const prices = recipe.map(r => calcMenuPrice(menuItem, r.ingredient, r.quantity_grams).sell);
...
function handleAdd(product) {
  ...
  if (!isCoffeeCategory(product.category) || recipe.length === 0) {
    const { hpp, sell } = calcMenuPrice(product);
  ...
  if (recipe.length === 1) {
    const r = recipe[0];
    const { hpp, sell } = calcMenuPrice(product, r.ingredient, r.quantity_grams);
    addToCart({ _type: 'menu', id: product.id, name: product.name,
...
function openBeanPicker(product) {
  ...
  product.recipe_ingredients
    ...
    .forEach(r => {
      const { sell, hpp } = calcMenuPrice(product, r.ingredient, r.quantity_grams);
      const btn = document.createElement('button');
...
async function processOrder() {
  ...
  const txItems = cart.map(item => ({
    ...
    recipe_qty_grams: item.recipeQty       || 0,
